...
import pgportfolio.marketdata.globaldatamatrix2 as gdm
import numpy as np
import pandas as pd
import logging
...
class DataMatrices2:
    def __init__(self, start, end, period, batch_size=50, volume_average_days=30, buffer_bias_ratio=0,
        ...
        self.__history_manager = gdm.HistoryManager2(online=online)
        if market == "poloniex":
    ...
    def create_from_config(config):
        ...
        start = (input_config["start_date"])
        end = (input_config["end_date"])
        return DataMatrices2(start=start,
                            end=end,
