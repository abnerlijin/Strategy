...
class NeuralNetWork:
    def __init__(self, feature_number, rows, columns, layers, device):
        ...
        self.input_num = tf.placeholder(tf.int32, shape=[]) #lijin: batch
...
class CNN(NeuralNetWork):
    ...
    def _build_network(self, layers):
        ...
        network = network / network[:, :, -1, 0, None, None] #lijin:归一化
        for layer_number, layer in enumerate(layers):
            if layer["type"] == "DenseLayer":
                ...
                width = network.get_shape()[2]  # [batch, assets, window, features]
                height = network.get_shape()[1]
                features = network.get_shape()[3]
                network = tf.reshape(network, [self.input_num, int(height), 1, int(width*features)]) # [batch, assets, 1，window*features]
                w = tf.reshape(self.previous_w, [-1, int(height), 1, 1])
                network = tf.concat([network, w], axis=3)  # [batch, assets, 1，window*features+1]
                ...
                network = tf.concat([btc_bias, network], 1) # [batch, assets+1]
                self.voting = network
